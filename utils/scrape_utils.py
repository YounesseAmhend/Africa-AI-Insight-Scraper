...
class ScrapeUtils:
    ...
    def scrape_news(url: str):
        ...
        page_url = soup.select_url(base_url=url,css_selector=str(general_selector["link"]))
    ...
    def scrape_news_detail(
        ...
        try:
            ...
            image_url = soup.select_url(base_url=base_url, css_selector=str(page_selector["image_url"]))
            ...
            author_name = soup.select_text(
                str(page_selector["author"]["name"]) if isinstance(page_selector["author"], dict) else None
            ) 
            author_link= soup.select_url(
                base_url = base_url,
                css_selector=str(page_selector["author"]["link"]) if isinstance(page_selector["author"], dict) else None
            ) 
            author_image_url = soup.select_url(
                base_url = base_url,
                css_selector=str(page_selector["author"]["image_url"])  if isinstance(page_selector["author"], dict) else None
            )
            end_time = time.time()
            logging.info(
            ...
            if body and post_date:
                logging.info(f"Body text length: {len(body)} characters")
